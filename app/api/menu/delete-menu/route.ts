...
export async function POST(req: NextRequest) {
  try {
    ...
    const menuId = body.menuId; // Changed from itemId to menuId
    ...
    if (!menuId) {
      return new Response(JSON.stringify({ message: "Menu ID is required" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    // Delete only if the menu belongs to the authenticated user
